                    ...
                    RNTesseractOcr.recognize(path, 'LANG_ENGLISH', tessOptions)
                      .then((result) => {
                        ...
                        if(this.state.ReceiptName.toString() === null || this.state.ReceiptName.toString() === "" || this.state.ReceiptName.toString() === " "){
                            var resultTitle = result.substr(0, 11)
                            this.setState({ReceiptName: resultTitle})
                        } else {
                            // DO NOTHING AND KEEP THE USER INPUT TITLE
                        }
                        ...
                        }else if(result.indexOf('party') > -1 || result.indexOf('pub') > -1 || result.indexOf('wine') > -1 || result.indexOf('beer') > -1 || result.indexOf('music') > -1){
                        ...
                        }else if(result.indexOf('invoice') > -1 || result.indexOf('bill') > -1 || result.indexOf('medic') > -1 || result.indexOf('care') > -1){
                        ...
                        if(Math.abs(resultTotal) != "NaN"){
                            resultTotal = resultTotal.replace(" ", "")
                            resultTotal = resultTotal.replace(' ', '')
                            resultTotal = (Math.round(resultTotal * 100) / 100).toFixed(2);
                            this.setState({ Total: resultTotal });
                        } else {
                            Alert.alert("Error Reading Total '" + resultTotal +"' As Money")
                        }
                        // UPDATE WITH RANGED REGEX [1-2] TO REDUCE LINES OF CODE
                        result = result.replace("/", "-");result = result.replace("/", "-");result = result.replace("/", "-");result = result.replace("/", "-");result = result.replace("/", "-");
                        ///^(0?[1-9]|[12][0-9]|3[01])[-](0?[1-9]|1[012])[-]\\d{4}$/g
                        var regDate = new RegExp(/(0?[1-9]|[12][0-9]|3[01])[-](0?[1-9]|1[012])[-]\\d{4}/);
                        var indexDate = result.search(this.regDate)
                        if(!(indexDate == null || indexDate < 0)){
                            var foundDate = result.substr(indexDate)
                            foundDate = foundDate.substr(0, 10)
                            foundDate = foundDate.replace(" ", "")
                            foundDate = foundDate.replace(' ', '')
                            foundDate = this.getParsedDate(foundDate);
                            if(foundDate != 'Invalid date'){
                                this.setState({ date: foundDate });
                            }
                        } else {
                            console.log("Error Finding Date at index: " + indexDate)
                        }
